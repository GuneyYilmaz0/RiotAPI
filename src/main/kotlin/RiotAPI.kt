...
class RiotAPI(private val apiKey: String, private val region: Region, private val server: Server) {
    ...
    fun getAccountByNameAndTag(string: String): Account =
        getAccountByNameAndTag(string.split("#")[0], string.split("#")[1])

    fun getAccountByNameAndTag(name: String, tag: String): Account {
        val url = "https://${region.code}.api.riotgames.com/riot/account/v1/accounts/by-riot-id/${
            name.replace(
                " ",
                "%20"
            )
    ...
    fun getMatchIdsByAccount(account: Account): List<String> {
        val url =
            "https://${region.code}.api.riotgames.com/lol/match/v5/matches/by-puuid/${account.puuid}/ids?start=0&count=20&api_key=$apiKey"
        val json = JsonParser.parseString(Utils.getJsonFromUrl(url))
